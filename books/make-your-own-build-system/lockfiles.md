...
minimake では、外部の依存関係としてツールのバージョンを管理することを考えます。たとえば、GCC のバージョンが異なると、生成されるバイナリが変わる可能性があります[^abi]。
...
ビルド前に、ロックファイルと現在の環境が一致しているかを検証します。この検証があるからこそ環境の違いを早期に検出できます。
...
ロックファイルは再現可能なビルドの第一歩です。同じロックファイルを使えば、異なる環境でも同じ条件でビルドできることが保証されます。次の章で扱うビルドキャッシュや再現可能なビルドとも密接に関係している機能です。
