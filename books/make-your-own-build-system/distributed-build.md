...
並列実行可能な部分を見つけるには、各ターゲットの「レベル」（依存の深さ）を計算します。同じレベルのターゲットは並列にビルドできます[^critical-path]。
...
並列ビルドをさらに発展させると、複数のマシンにビルドを分散させる「分散ビルド」になります[^distcc]。
...
ビルドシステムでは、依存関係による制約が「並列化できない部分」に相当します。クリティカルパス上のビルドは並列化できないため、全体の高速化に限界があります。
